...
namespace Opsive.UltimateInventorySystem.Editor.Managers.UIDesigner
{
    ...
    public class LayoutGroupOption : GenericGridEditorOption
    {
        ...
        protected ObjectField m_ItemViewSlotField;
        protected Button m_ReplaceAllSlots;
        ...
        {
            Add(new SubTitleLabel("Grid Slots"));
            m_ItemViewSlotField = new ObjectField("Grid Slots Prefab");
            m_ItemViewSlotField.objectType = typeof(GameObject);
            m_ItemViewSlotField.RegisterValueChangedCallback(evt =>
            {
                if (m_GridBase?.GridEventSystem == null) {
                    Debug.LogWarning("The Grid cannot be found.");
                    return;
                }
                m_GridBase.GridEventSystem.ButtonPrefab = evt.newValue as GameObject;
                EditorUtility.SetDirty(m_GridBase);
            });
            Add(m_ItemViewSlotField);

            m_ReplaceAllSlots = CreateButton("Replace All Slots", this, ReplaceAllSlots);
            ...
            m_LayoutGroupField.RegisterValueChangedCallback(evt =>
            {
                ...
                switch (option) {
                    case GridLayoutEnum.Grid:
                        var layoutGroup = m_Content.gameObject.GetComponent<LayoutGroup>();
                        if (layoutGroup != null) {
                            if (layoutGroup is GridLayoutGroup) {
                                return;
                            } else {
                                RemoveComponent(layoutGroup);
                            }
                        }
        ...
        private void ReplaceAllSlots()
        {
            var gridSize = m_GridBase.GridSize;
            ResizeGrid(gridSize, true);
        }
        
        private void ResizeGrid(Vector2Int newSize)
        {
            ResizeGrid(newSize, false);
        }

        private void ResizeGrid(Vector2Int newSize, bool forceReplace)
        {
            if (OnValidateUtility.IsPrefab(m_GridBase)) {
                Debug.LogWarning("Cannot resize Grid if it is a Prefab. Unpack, select the grid and try again");
                return;
            }
            m_GridBase.GridSize = newSize;
            m_GridBase.GridEventSystem.SpawnButtons(m_GridBase.GridSize, forceReplace);
        ...
        public override void Refresh()
        {
            ...
            m_ItemViewSlotField.SetValueWithoutNotify(m_GridBase?.GridEventSystem?.ButtonPrefab);
            ...
            if (m_LayoutGroup == null) {
                ...
                m_LayoutGroupField.SetValueWithoutNotify(GridLayoutEnum.Other);
            }
        ...
        private void BuildAction()
        {
            ...
            if (replace) {
                ...
                if (m_Navigator is GridNavigatorWithScrollView gridNavigatorWithScrollView) {
                    ...
                    var mask = gridNavigatorWithScrollView.m_Content.transform.parent.GetComponent<Mask>();
                    if (mask != null) {
                        RemoveComponent(mask.graphic);
